...
## /project/salhia_618/POETIC_Dragen_Data/hmcnc
...
rule all:
    input:
        expand(f"{OUTDIR}/{{sample}}/{{sample}}.cn.bed", sample=SAMPLES),
        expand(f"{OUTDIR}/{{sample}}/{{sample}}.cn.DUP.bed", sample=SAMPLES),
        expand(f"{OUTDIR}/{{sample}}/{{sample}}.cn.DEL.bed", sample=SAMPLES),
        expand(f"{OUTDIR}/{{sample}}/{{sample}}.gene_count.bed", sample=SAMPLES),
        expand(f"{OUTDIR}/{{sample}}/{{sample}}.cn_gene.bed", sample=SAMPLES)
...
rule bam_to_bed:
    ...
    output:
        bed=f"{OUTDIR}/{{sample}}/{{sample}}.samTo.bed"
    params:
        samtoBed=TOOLS["samToBed"],
        threads=config["threads"]
    threads: config["threads"]
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.bam_to_bed.log"
    shell:
        """
...
rule count_reads_in_bins:
    ...
    output:
        counts=f"{OUTDIR}/{{sample}}/{{sample}}.read_counts_{BIN_SIZE}bp.bed.gz"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.count_reads_in_bins.log"
    shell:
        """
...
rule calculate_mean_coverage:
    input:
        counts=f"{OUTDIR}/{{sample}}/{{sample}}.read_counts_{BIN_SIZE}bp.bed.gz"
    output:
        mean_cov=f"{OUTDIR}/{{sample}}/{{sample}}.mean_cov.txt"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.calculate_mean_coverage.log"
    shell:
        """
        zcat {input.counts} | \
...
rule viterbi_analysis:
    input:
        ...
        mean_cov=f"{OUTDIR}/{{sample}}/{{sample}}.mean_cov.txt"
    output:
        viterbi_out=f"{OUTDIR}/{{sample}}/{{sample}}.viterout.txt"
    params:
        viterbi_tool=TOOLS["viterbi"],
        bin_size=BIN_SIZE,
        viterbi_params=VITERBI_PARAMS,
        output_prefix=f"{OUTDIR}/{{sample}}/{{sample}}"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.viterbi_analysis.log"
    shell:
        """
...
rule combine_counts_and_cn:
    input:
        ...
        viterbi_out=f"{OUTDIR}/{{sample}}/{{sample}}.viterout.txt"
    output:
        combined=f"{OUTDIR}/{{sample}}/{{sample}}.cn_{BIN_SIZE}bp.bed"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.combine_counts_and_cn.log"
    shell:
        """
...
rule merge_adjacent_bins:
    input:
        combined=f"{OUTDIR}/{{sample}}/{{sample}}.cn_{BIN_SIZE}bp.bed"
    output:
        merged=f"{OUTDIR}/{{sample}}/{{sample}}.cn.bed"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.merge_adjacent_bins.log"
    shell:
        """
...
rule filter_duplications:
    input:
        merged=f"{OUTDIR}/{{sample}}/{{sample}}.cn.bed"
    output:
        duplications=f"{OUTDIR}/{{sample}}/{{sample}}.cn.DUP.bed"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.filter_duplications.log"
    shell:
        """
...
rule filter_deletions:
    input:
        merged=f"{OUTDIR}/{{sample}}/{{sample}}.cn.bed"
    output:
        deletions=f"{OUTDIR}/{{sample}}/{{sample}}.cn.DEL.bed"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.filter_deletions.log"
    shell:
        """
...
rule annotate_cnv_gene_counts:
    ...
    output:
        gene_counts=f"{OUTDIR}/{{sample}}/{{sample}}.gene_count.bed"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.annotate_cnv_gene_counts.log"
    shell:
        """
...
rule annotate_cnv_genes:
    input:
        ...
        gene_bed=GENE_ANNOTATIONS
    output:
        cn_genes=f"{OUTDIR}/{{sample}}/{{sample}}.cn_gene.bed"
    log:
        f"{OUTDIR}/{{sample}}/logs/{{sample}}.annotate_cnv_genes.log"
    shell:
        """
        cat {input.duplications} {input.deletions} | sort -k1,1 -k2,2n | \
        intersectBed -F 1 -wb -wa -a stdin -b {input.gene_bed} | \
...
rule create_directories:
    output:
        directory(f"{OUTDIR}/{{sample}}/logs")
    shell:
        "mkdir -p {output}"
...
rule validate_inputs:
    input:
        ...
        gene_bed=GENE_ANNOTATIONS
    output:
        touch(f"{OUTDIR}/{{sample}}/.validation_complete")
    shell:
        """
        if [ ! -f {input.bam} ]; then
...
rule generate_summary:
    input:
        ...
        expand(f"{OUTDIR}/{{sample}}/{{sample}}.cn_gene.bed",  sample=SAMPLES)
    output:
        summary=f"{OUTDIR}/summary_statistics.txt"
    params:
        samples=SAMPLES,
        OUTDIR=OUTDIR
    shell:
