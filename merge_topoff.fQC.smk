...
def find_fastq_files():
    ...
    Matches on first 9 fields (before lane) + read orientation.
    ...
    for input_dir in INPUT_DIRS:
        ...
        for fastq in fastq_files:
            ...
            if not read_number or not lane_number or len(parts) < 9:
            ...
            matching_key = f"{'_'.join(parts[:9])}_{read_number}"
...
rule all:
    input:
        ...
        os.path.join(OUTPUT_DIR, "logs", "fastqc.log"),
...
rule fastqc:
    input:
        merged_files = expand(os.path.join(OUTPUT_DIR, "{output_file}"),
                            output_file=MERGE_INFO.keys())
    output:
        outdir = directory(os.path.join(OUTPUT_DIR, "fastqcOut")),
        log = os.path.join(OUTPUT_DIR, "fastqc.log"),
        flag = touch(os.path.join(OUTPUT_DIR, "fastqcPass"))
    log:
        os.path.join(OUTPUT_DIR, "logs", "fastqc.log")
    shell:
        """
        ...
        fastqc --threads 16 --outdir {output.outdir} {input.merged_files} &> {output.log}
        """
...
rule multiQC:
    input:
        fastqc_dir = os.path.join(OUTPUT_DIR, "fastqcOut"),
        fastqc_log = os.path.join(OUTPUT_DIR, "fastqc.log")
    output:
        outdir = directory(os.path.join(OUTPUT_DIR, "multiqc")),
        flag = touch(os.path.join(OUTPUT_DIR, "multiqcPass"))
    log:
        os.path.join(OUTPUT_DIR, "logs", "multiqc.log")
    shell:
        """
